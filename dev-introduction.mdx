...
When OTP is required, a successful credential check sends the one-time code and returns `204` with no body; the caller must then verify the OTP to receive tokens.
