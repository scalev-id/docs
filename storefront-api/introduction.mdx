...
7. Optionally validate a discount code with `GET /public/discount-codes/check?code=...`. Unknown or ineligible codes return `is_eligible: false`; the route only returns `404` when the store cannot be resolved.
