...
After guest checkout, public order reads, and public payment creation, the response includes the order data a storefront needs to show the buyer the next step: order identity, `public_order_url`, `payment_url`, status, totals, line items, shipping details, and payment fields such as `payment_method`, `sub_payment_method`, `payment_account_holder`, `payment_account_number`, `transferproof_url`, and `pg_payment_info`. In v3, those fields are returned directly in the JSON object, not inside the legacy `data` envelope.
