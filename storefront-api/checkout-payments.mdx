...
The hosted success page first derives a small display model from provider-specific `pg_payment_info`. This is not a separate API response shape. It is a useful reference if your storefront wants one payment renderer instead of branching directly on every provider payload:
...
The actual source fields come from the order and payment response. The hosted success page maps them this way:

| Provider | QR source | Payment number source | Account holder source | URL source |
| --- | --- | --- | --- | --- |
| `duitku` | `pg_payment_info.qrString` | `pg_payment_info.vaNumber` | empty | `pg_payment_info.paymentUrl` |
| `ipaymu` | `pg_payment_info.QrString` | `pg_payment_info.PaymentNo` | `pg_payment_info.PaymentName` | `pg_payment_info.Url` |
| `midtrans` GoPay | none | none | none | mobile `actions[].name == "deeplink-redirect"` |
| default/Xendit-style | `pg_payment_info.qr_string` or `payment_method.qr_code.channel_properties.qr_string` | legacy `pg_payment_info.account_numbner`, `payment_method.virtual_account.channel_properties.virtual_account_number`, or `payment_method.over_the_counter.channel_properties.payment_code` | `payment_method.virtual_account.channel_properties.customer_name` or `payment_method.over_the_counter.channel_properties.customer_name` | `pg_payment_info.invoice_url` or `pg_payment_info.redirect_url` |
...
Render `vaNumber` as `No Pembayaran`, show `vaAccountHolder` when present, and provide a copy action. When the provider is `ipaymu`, also show the payment tutorial link.
