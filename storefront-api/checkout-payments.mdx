...
On success, the response includes the created order data, including `secret_slug`, `public_order_url`, `payment_url`, status, totals, line items, shipping details, and payment fields. Use `secret_slug` to read or update the order. Use `payment_url` only as a hosted fallback if your storefront does not render the payment instructions itself.
...
They use `Authorization: Bearer <customer_access_token>`, not the Storefront API key. Addresses, payment methods, shipping options, and summary use the same checkout preparation fields as guest checkout. Confirm returns the same public order response used by guest checkout and public order reads, so every checkout completion response has the same order fields.

Customer checkout summary uses the selected courier service, warehouse, destination, payment method, and customer cart items to recompute shipping cost server-side. A client-supplied `shipping_cost` is only a compatibility value and is not the authoritative amount.
