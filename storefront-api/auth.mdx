---
title: "Auth"
description: "Use Storefront API keys, guest cart tokens, and customer JWTs from a browser storefront."
---
...
If the store requires OTP, the response is `204` with an empty body. Show the one-time-code field and call:
...
Password reset email links use the validated browser `Origin` when the request comes from an allowed Storefront API origin. Requests without an `Origin` header fall back to the hosted storefront custom domain.
