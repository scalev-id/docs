...
If the store can complete login directly, the response is `200` with customer tokens:
...
If the store requires OTP, the response is `200` with an OTP challenge message:
...
Show the one-time-code field and call:
