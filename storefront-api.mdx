...
You get this token from the first guest cart response. Call `GET /public/cart` or `POST /public/cart/items` without a guest token, then read `X-Guest-Token` or `X-Scalev-Guest-Token` from the response headers.
...
The same token is exposed through both guest-token headers. Use either one on later requests.
...
Allowed-origin CORS preflight includes `X-Scalev-Storefront-Api-Key`, `X-Guest-Token`, `X-Scalev-Guest-Token`, `Authorization`, and `Content-Type`.
