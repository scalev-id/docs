...
3. Jalankan Claude Code lalu ketik perintah slash `/mcp`. Claude Code menampilkan server Scalev dan membuka alur persetujuan OAuth Scalev di browser. Masuk, pilih bisnis yang ingin Anda berikan akses ke asisten, tinjau izin yang diminta, lalu setujui.

4. Setelah disetujui, tool Scalev tampil di panel `/mcp` dan dapat dipanggil dari prompt. Nama tool mengikuti pola `mcp__scalev__<tool>` (contoh: `mcp__scalev__list_landing_pages`).
