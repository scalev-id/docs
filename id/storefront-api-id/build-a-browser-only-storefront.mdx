---
title: "Bangun storefront browser-only dengan Storefront API v3"
description: "Ikuti arsitektur frontend dan alur end-to-end canonical untuk membangun storefront langsung ke Storefront API v3."
---

Gunakan panduan ini saat Anda membutuhkan satu jalur implementasi storefront, bukan menyatukan alur dari banyak halaman endpoint.

Storefront API v3 dirancang agar storefront bisa di-host sebagai client browser statis, misalnya di Cloudflare Pages, Vercel, Netlify, atau CDN lain, tanpa membuat backend custom atau proxy same-origin. Browser memanggil `https://api.scalev.com` secara langsung.

## Arsitektur

Gunakan aturan ini untuk storefront browser:

- Gunakan `unique_id` store di URL runtime storefront publik.
- Gunakan storefront public API key yang publishable di browser.
...
Runtime Storefront memakai `unique_id` store. Endpoint setup Storefront API adalah endpoint bisnis terautentikasi. Endpoint setup bisa menerima numeric store ID lama untuk kompatibilitas, tetapi storefront browser sebaiknya dikonfigurasi dengan `unique_id` store.

## Helper fetch browser minimal
...
Jangan tampilkan input token reset manual. Frontend tidak boleh meminta user copy dan paste reset token. Hostname reset dipilih dari registered allowed origin saat request browser menyertakan `Origin`.
...
- melakukan semua hal di atas dari browser tanpa backend custom
