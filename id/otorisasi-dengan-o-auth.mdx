...
Merchant dapat memilih beberapa bisnis dalam satu persetujuan OAuth jika keanggotaan mereka di setiap bisnis mengizinkan otorisasi aplikasi. Scalev membuat atau memperbarui satu instalasi untuk setiap bisnis yang dipilih. Scope, pengaturan webhook, dan persetujuan tag billing yang sama berlaku untuk semua bisnis yang dipilih pada alur saat ini.
...
Untuk token OAuth, endpoint ini mengembalikan identitas token dan user serta daftar bisnis yang terhubung ke token:
...
`/v3/me` adalah identitas token dan user. Endpoint ini bukan identitas bisnis yang sedang dipilih. Status instalasi bisnis dan scope yang disetujui berada di dalam `connected_businesses[]`.

### Pilih Bisnis untuk Request API

Request API `/v3` yang bersifat business-scoped selalu berjalan pada satu instalasi bisnis yang dipilih.
...
- jika instalasi bisnis yang dipilih dicabut atau dinonaktifkan, bisnis tersebut gagal tetapi bisnis lain yang terhubung tetap dapat digunakan
...
Scalev memakai business unique ID di selector API. Internal token dapat memakai ID database, tetapi integrasi Anda harus memakai `unique_id` yang dikembalikan oleh `/v3/me`.
...
`/v3/oauth/installation/status` menentukan satu snapshot instalasi. Jika Anda mengirim referensi token multi-bisnis, Scalev mengembalikan error karena endpoint ini tidak memiliki selector bisnis. Gunakan `/v3/me` untuk melihat daftar bisnis yang terhubung ke token multi-bisnis.
...
- refresh memuat ulang bisnis yang terhubung ke token dan hanya mempertahankan instalasi yang aktif dan enabled
