...
`store.payment_methods` berisi payment methods aktif setelah Scalev memfilter pengaturan store berdasarkan kapabilitas payment bisnis. Gunakan `store.sub_payment_methods` untuk submethod VA yang aktif saat tersedia.

Bank virtual account bukan value `paymentMethod` terpisah. Untuk BRI VA, kirim `paymentMethod: "va"` dan `subPaymentMethod: "BRI"`. Jangan kirim string gabungan seperti `briva`, `bri_va`, atau `va_bri`.

Untuk bank transfer manual, render pilihan per rekening dari `store.payment_accounts` dengan `method` `bank_transfer`. Jangan tampilkan satu pilihan `bank_transfer` generik tanpa rekening. Khusus pilihan rekening `bank_transfer` ini, tampilkan nama bank dari `payment_account.financial_entity.name`; gunakan `payment_account.financial_entity.code` hanya sebagai fallback. `account_holder` adalah pemilik rekening, bukan nama bank. UI checkout bisa menggabungkan method regular dari `store.payment_methods` dengan rekening bank transfer tersebut, lalu submit rekening terpilih sebagai `paymentAccountUniqueId` atau `payment_account_unique_id`.
...
`pageUniqueId` otomatis memakai halaman saat ini, dan `domain` otomatis memakai hostname saat ini. Untuk VA, kirim `paymentMethod: "va"` dan kode bank terpilih sebagai `subPaymentMethod`, misalnya `subPaymentMethod: "BRI"`. Value persis yang ada di `store.sub_payment_methods` juga dinormalisasi menjadi `va` untuk kompatibilitas, tetapi jangan mengarang value gabungan seperti `briva`. Untuk bank transfer manual, kirim `paymentMethod: "bank_transfer"` dan rekening terpilih sebagai `paymentAccountUniqueId` atau `payment_account_unique_id`.
...
Opsional untuk memvalidasi bentuk payload order tanpa membuat order. Helper ini berguna untuk preflight check atau debugging, tetapi tidak wajib dipanggil sebelum `createOrder`. Pada flow submit normal, panggil `createOrder` dan tangani error validasinya.
...
Membuat public order sungguhan dan memvalidasi payload. Panggil method ini hanya setelah pengunjung sengaja submit form. Kamu tidak perlu memanggil `validateOrder` terlebih dulu.
...
- Payment fields: `paymentMethod`, `paymentAccountUniqueId`, `payment_account_unique_id`, dan `subPaymentMethod`
...
Untuk VA, submit payment method dan bank secara terpisah: `paymentMethod: "va"` dan `subPaymentMethod` diisi salah satu value `store.sub_payment_methods`, misalnya `"BRI"`. Jangan submit BRI VA sebagai `paymentMethod: "briva"` atau string method gabungan lain. Untuk bank transfer manual, kirim `paymentMethod: "bank_transfer"` dan unique id rekening terpilih.

Saat payment method terpilih adalah `bank_transfer`, runtime mewajibkan `paymentAccountUniqueId` atau `payment_account_unique_id` sebelum mengirim request order. Gunakan object terpilih dari `store.payment_accounts`; runtime meneruskan `unique_id` object tersebut sebagai `payment_account_unique_id` untuk order API. Jika id rekening bank, financial entity id, nama pemilik rekening, atau nomor rekening ikut dikirim, runtime mengabaikannya.

Jangan membuat order otomatis saat halaman dibuka.

Setelah `createOrder` berhasil, HTML Checkout Page otomatis men-trigger event analytics submit form yang sudah dikonfigurasi di sisi client dan server. Event hanya dikirim untuk pixel analytics yang sudah dikonfigurasi pada page. HTML Checkout Page baru di editor sudah terisi awal dengan Meta dan TikTok `InitiateCheckout`, SnackVideo client `formSubmit`, dan SnackVideo server `EVENT_FORM_SUBMIT`; kamu bisa mengubahnya sebelum menyimpan page. Jangan mengirim ulang event submit yang sudah dikonfigurasi kecuali memang perlu event custom tambahan.

Pilih sendiri jenis redirect setelah order. Runtime membuat order dan men-trigger submit analytics; runtime tidak melakukan navigasi otomatis. Baca `Scalev.data.get().after_checkout` jika tersedia, lalu ikuti [Aksi setelah order HTML Mode](/id/aksi-setelah-order-html-mode).
...
Method runtime me-reject `Promise` dengan JavaScript `Error`. Method ini tidak mengembalikan HTTP error envelope yang stabil untuk ditampilkan ke customer.

Gunakan `error.message` untuk copy UI. Gunakan `error.status`, `error.data`, dan `Scalev.diagnostics.get()` hanya untuk debugging atau handling lanjutan. Jangan tampilkan `error.data.error` langsung; tergantung kegagalan upstream, field itu bisa berupa string, boolean, object, atau map error per field.
...
try {
  ...
  showFormError(error.message || "Order tidak dapat dibuat.");
...
Jangan memanggil URL proxy runtime `/api/v2/...` langsung dari kode halaman. Gunakan method `Scalev.checkout.*` supaya error validasi dinormalisasi sebelum sampai ke UI.
