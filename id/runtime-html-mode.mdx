...
`pageUniqueId` otomatis memakai halaman saat ini, dan `domain` otomatis memakai hostname saat ini. Value payment yang ada di `store.sub_payment_methods` dinormalisasi menjadi `va`. Untuk bank transfer manual, kirim `paymentMethod: "bank_transfer"` dan rekening terpilih sebagai `paymentAccountUniqueId` atau `payment_account_unique_id`.
...
Seperti helper diskon dan shipping, submethod VA yang aktif di `store.sub_payment_methods` dikirim sebagai `payment_method: "va"` dengan submethod terpilih. Untuk bank transfer manual, kirim `paymentMethod: "bank_transfer"` dan unique id rekening terpilih.
