...
- Bank transfer manual dianggap sebagai `bank_transfer`. Saat membuat order, sertakan rekening bank terpilih sebagai `paymentAccountUniqueId` atau `payment_account_unique_id`.
- Jika status order adalah `draft`, jalur instruksi pembayaran memakai **Self Hosted Orderan / Invoice** (`order_page`).
- Jika halaman dirender di dalam iframe, kirim target URL ke parent window dan jangan langsung menavigasi iframe.
...
Setelah `Scalev.checkout.createOrder(payload)` berhasil, HTML Checkout Page otomatis men-trigger event analytics submit form yang sudah dikonfigurasi untuk pixel analytics pada page. Redirect tetap perlu ditangani sendiri, jadi lakukan redirect setelah `createOrder` selesai dan jangan menduplikasi event submit yang sudah dikonfigurasi.
