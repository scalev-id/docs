...
```text
X-Guest-Token: 7e7f8c12-2e1d-4b31-97ad-5ca4f8d1c2d0
```

Anda mendapatkan token ini dari response guest cart pertama. Panggil `GET /public/cart` atau `POST /public/cart/items` tanpa guest token, lalu baca `X-Guest-Token` atau `X-Scalev-Guest-Token` dari response header.
...
const guestToken =
  ...
  response.headers.get("X-Guest-Token");

localStorage.setItem("scalev_guest_token", guestToken);
```

Token yang sama diekspos melalui kedua guest-token header. Gunakan salah satunya pada request berikutnya.
...
CORS preflight untuk origin yang diizinkan mencakup `X-Scalev-Storefront-Api-Key`, `X-Guest-Token`, `X-Scalev-Guest-Token`, `Authorization`, dan `Content-Type`.
