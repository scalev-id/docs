...
Jika store dapat menyelesaikan login secara langsung, response adalah `200` dengan token customer:
...
Jika store mewajibkan OTP, response adalah `200` dengan pesan OTP challenge:
...
Tampilkan input one-time code lalu panggil:
