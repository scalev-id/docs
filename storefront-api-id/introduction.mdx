...
Setelah guest checkout, membaca public order, atau membuat instruksi payment, response berisi data order yang dibutuhkan storefront untuk menampilkan langkah berikutnya ke pembeli: identity order, `public_order_url`, `payment_url`, status, total, item, detail pengiriman, dan field payment seperti `payment_method`, `sub_payment_method`, `payment_account_holder`, `payment_account_number`, `transferproof_url`, dan `pg_payment_info`. Di v3, field tersebut ada langsung di object JSON, bukan di dalam envelope `data` lama.
