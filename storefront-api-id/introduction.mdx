...
5. Buat order dengan `POST /public/guest-checkout`.
6. Baca order melalui `GET /public/orders/{secret_slug}`.
7. Panggil `POST /public/orders/{secret_slug}/payment`; gunakan ulang `payment_url` yang dikembalikan atau yang sudah ada.
8. Untuk akun customer, mulai dari `/public/auth/otp/send`, lalu panggil `/customers/me/*` dengan customer JWT.
...
Untuk response payment, prioritaskan `payment_url` saat tersedia. Metode manual seperti `bank_transfer` dan `cod` dapat mengembalikan `pg_payment_info` kosong dan memakai instruksi pada public order. Virtual account, QRIS, e-wallet, card, dan invoice dapat mengembalikan data provider-specific di `pg_payment_info`; gunakan hanya untuk UI khusus metode tersebut dan tetap jadikan `payment_url` sebagai URL redirect atau instruksi utama.
