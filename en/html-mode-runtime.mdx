...
`pageUniqueId` defaults to the current page, and `domain` defaults to the current hostname. Payment values listed in `store.sub_payment_methods` are normalized to `va`. For manual bank transfer, send `paymentMethod: "bank_transfer"` and the selected account as `paymentAccountUniqueId` or `payment_account_unique_id`.
...
As with discount and shipping helpers, enabled VA submethods in `store.sub_payment_methods` are sent as `payment_method: "va"` with the selected submethod. For manual bank transfer, send `paymentMethod: "bank_transfer"` and the selected account unique id.
