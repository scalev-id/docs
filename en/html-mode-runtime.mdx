...
After `createOrder` succeeds, choose the post-order redirect yourself. The runtime creates the order only; it does not navigate automatically. Read `Scalev.data.get().after_checkout` when present, then follow [HTML Mode checkout success types](/en/html-mode-checkout-success-paths).
