...
description: "Connect AI assistants to your Scalev business with OAuth."
---

Scalev MCP lets AI assistants that support remote MCP connect to your Scalev business. Use it when you want an assistant to inspect business data, find available Scalev API v3 actions, or run approved business API requests through a permissioned OAuth connection.
...
When you connect an assistant, Scalev opens an OAuth approval flow. You sign in, choose the business, review the requested permissions, and approve access.

After approval, the assistant calls the Scalev MCP connector. The connector forwards the OAuth token to Scalev API v3. Scalev validates the token, resolves the business and user, checks scopes, and runs the requested API action.
...
- `get_me`: shows the connected Scalev business, user, OAuth app, and permission context.
...
- showing the connected business and user
...
6. Choose the business you want the assistant to access.
...
The first connection may request the full set of machine-safe Scalev API v3 permissions so the assistant can discover and use the available business API actions. Dashboard-only permissions are not part of MCP approval.
...
After revocation, the assistant must reconnect and complete OAuth approval again before it can call Scalev.
...
Make sure you selected the correct Scalev business during approval and that the business has landing pages available to your user.
