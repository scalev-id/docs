...
3. Start Claude Code and run the `/mcp` slash command. Claude Code shows the Scalev server and triggers the Scalev OAuth approval flow in your browser. Sign in, choose the business or businesses you want the assistant to access, review the requested permissions, and approve.

4. After approval, the Scalev tools appear in the `/mcp` panel and are addressable from prompts. Tool names follow the `mcp__scalev__<tool>` pattern (for example, `mcp__scalev__list_landing_pages`).
