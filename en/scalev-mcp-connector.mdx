...
`get_me` returns `connected_businesses[]`. If the token is connected to more than one business, the assistant must pass `business_unique_id` on `get` and `execute`. If the token is connected to one business, the assistant can omit it.
...
A single OAuth connection can authorize multiple Scalev businesses when your user role permits app authorization in each business. Scalev keeps one installation per business, but the assistant can use one OAuth token for the connected set.
