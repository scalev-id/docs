...
Attach existing analytics pixels to a page display by sending the Scalev pixel record IDs on the display payload. These are the numeric Scalev record IDs, not the provider pixel code such as a Meta Pixel ID.
