---
title: "Build a browser-only storefront with Storefront API v3"
description: "Follow the canonical frontend architecture and end-to-end flow for building a storefront directly against Storefront API v3."
---

Use this guide when you want one storefront implementation path instead of piecing the flow together from endpoint pages.

Storefront API v3 is designed so a storefront can be hosted as a static browser client, for example on Cloudflare Pages, Vercel, Netlify, or any CDN, without building a custom backend or same-origin proxy. The browser calls `https://api.scalev.com` directly.

## Architecture

Use these rules for browser storefronts:

- Use the store `unique_id` in public storefront runtime URLs.
- Use a publishable storefront public API key in the browser.
...
Storefront runtime uses the store `unique_id`. Storefront setup endpoints are authenticated business endpoints. The setup endpoints may accept legacy numeric store IDs for compatibility, but the browser storefront should be configured with the store `unique_id`.

## Minimal browser fetch helpers
...
Do not show a manual reset token input. The frontend should never ask the user to copy and paste the reset token. The reset hostname is selected from the registered allowed origin when the browser request includes `Origin`.
...
Manual bank transfer may use `store.payment_accounts` when no specific order-level account has been selected. Hosted or provider-backed methods may use `payment_url` or `pg_payment_info`. `gross_revenue` is the amount the buyer pays.
...
- do all of the above from the browser without a custom backend
