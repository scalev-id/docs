...
<Update label="May 2026" description="Customer auth token responses" tags={["API"]}>
  ...
  the store does not require OTP, or `204` when an OTP has been sent.
  `POST /public/auth/login/verify` and `POST /public/auth/jwt/refresh` also
  return the typed access and refresh tokens. Send the access token as
  `Authorization: Bearer <token>` to `/customers/me/*` routes. Password reset
  links now follow the browser `Origin` when it matches a registered
  Storefront API allowed origin, and fall back to the hosted storefront
...
<Update label="May 2026" description="Browser-only Storefront API" tags={["API", "Dashboard", "Docs"]}>
  ...
  reset links use the validated storefront browser origin, Storefront API
