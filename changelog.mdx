...
<Update label="May 2026" description="Typed customer endpoint responses" tags={["API"]}>
  ...
  content, and progress responses. `POST /customers/me/cart/checkout` is now
  deprecated; new integrations should use
  `POST /customers/me/checkout/confirm`. The discount-code validation route
  is `GET /public/discount-codes/check` and accepts `code`, `payment_method`,
  `net_product_price`, `gross_revenue`, and `shipping_cost` as query
  parameters; the legacy `discount_code` query name is still accepted as an
  alias.
</Update>
